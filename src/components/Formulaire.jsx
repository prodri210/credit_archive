...
import logo from "/photo_2024-12-13_14-23-38.jpg";
...
const Formulaire = ({ onSubmit }) => {
  ...
  useEffect(() => {
      ...
      countries.forEach((country) => {
        ...
        countryData[countryCode] = {
          ...
          ...Object.keys(translations).reduce((acc, lang) => {
            acc[lang] = translations[lang].common;
            return acc;
          }, {}),
        };
      });
    ...
    const fetchData = async () => {
      ...
      switch (i18n.language) {
        case "de":
          countriesData = Object.values(transformedData).map(
            (country) => country.deu
          );
          break;
        case "fr":
  ...
  const handleSubmit = (e) => {
    ...
    formData.remboursement = `${formData.remboursement} ${t("monthLabel")}`;
  ...
  if (!isClient || loadingCountries) {
    return (
      <div className="h-screen flex items-center justify-center">
        <p className="text-2xl font-semibold text-green-500">Loading...</p>
      </div>
    );
  }
